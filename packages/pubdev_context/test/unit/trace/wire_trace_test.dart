...
void main() {
  ...
  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('pubdev_context_wire_trace_test_');
  });
  ...
  WireTrace openTrace({
    ...
    concurrency: 5,
    cacheDir: '~/.cache/pubdev_context',
    pid: pid,
    clock: clock,
    onWarning: onWarning,
  );

  List<String> sessionFiles() => tempDir
      .listSync()
      .whereType<File>()
  ...
  group('session file and header', () {
    ...
    test('writes the session-header block as the first thing in the file', () {
      ...
      final bar = '═' * 72;
      expect(lines[0], bar);
      expect(lines[1], ' Wire Trace — pubdev_context 0.0.0-test');
  ...
  group('event formatting', () {
    ...
    test('HTML response renders the HTML → md sizes and the markdown preview', () {
      ...
      expect(
        lines,
        contains('14:03:11.204  #001    ← pub  200 /packages/foo/changelog  '
    ...
    test('tarball response renders size and file count as metadata only', () {
      ...
      expect(
        lines,
        contains('14:03:11.204  #001    ← pub  200 '
    ...
    test('ok result renders summary/duration/size and body preview', () {
      ...
      expect(
        lines,
        contains('14:03:11.204  #001  → LLM   result  search_packages   ok   '
    ...
    test('error result renders ERROR code and error continuation', () {
      ...
      expect(
        lines,
        contains('14:03:11.204  #019  → LLM   result  get_package   '
    ...
    test('retry renders the ⚠ marker, attempt count and backoff', () {
      ...
      expect(
        body(),
        contains('14:03:11.204  #055    ⚠ pub  503 /api/search  '
    ...
    test('cache hit renders the ⚡ marker, key and age', () {
      ...
      expect(
        body(),
        contains('14:03:11.204  #002    ⚡ cache hit  get_package:http   '
