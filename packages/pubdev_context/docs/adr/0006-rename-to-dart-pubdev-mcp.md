...
**Migration:** publish `dart_pubdev_mcp` fresh at `1.0.0` (clean break, matches the PRD's framing of V1 as the definitive architecture, not a continuation of the 0.x PoC line). Mark `pubdev_context` discontinued on pub.dev with a description/README pointer to `dart_pubdev_mcp`. Existing `pubdev_context` installs keep working (nothing is unpublished); new users and search traffic get redirected.
...
- At cutover: rename the directory, `pubspec.yaml` `name:`/`executables:`, `bin/` entry point, `Implementation.name` in `server.dart`, and `.mcp.json`; publish `dart_pubdev_mcp@1.0.0`; discontinue `pubdev_context` on pub.dev.
