...
String? currentCorrelationId() => Zone.current[wireTraceZoneIdKey] as String?;
...
final class WireTrace {
  ...
  factory WireTrace.open({
    ...
    try {
      ...
      warn(
        '[$kMcpServerIdentity] Wire Trace disabled: cannot write to "$directoryPath".',
      );
      return WireTrace._(
  ...
  void _writeHeader({
    ...
    _write(bar);
    _write(' Wire Trace — $kMcpServerIdentity $serverVersion');
    _write(' session started ${_formatTimestampDate(start)}   pid $pid');
    _write(
      ' config: max-preview=${maxPreviewBytes}B  '
